...
import { apiCreateTour } from '../../services/tour'
...
const CreateTour = () => {
    ...
    useEffect(() => {
        const letcreate = async () => {
            const response = await apiCreateTour(queries)
            setcheck(false)
            if (response?.data.err ===0){
    ...
    const handletour = () => {
        ...
        setqueries(prev => ({...prev,staff}))
        setqueries(prev => ({...prev,phone}))
        setcheck(true)
        ...
        <div className='px-6'>
            ...
            <div className='py-4 flex flex-col gap-4 flex-auto justify-center'>
                <div className='flex flex-col gap-2'>
                    ...
                    <input defaultValue='Giải ' onChange={(e) => setname(e.target.value)} className='w-[760px] h-[36px] rounded-md'/>
                </div>
                ...
                <button className='bg-second w-[760px] h-[40px]' type='button' onClick={handletour}>Tạo giải</button>
            </div>
        </div>
    )
}
